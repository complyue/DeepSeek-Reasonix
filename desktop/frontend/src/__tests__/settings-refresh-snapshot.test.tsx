...
// Models > Agent runtime: the compaction slider is directly visible, shows the
// effective token threshold, and persists a preset through the batch API.
...
const thresholdCalls: [number, number, number, number][] = [];
window.go = {
  main: {
    App: {
      Settings: async () => compactSettings,
      FetchAllProviderModels: async () => ({}),
      SetCompactionThresholds: async (soft: number, snip: number, compact: number, force: number) => {
        thresholdCalls.push([soft, snip, compact, force]);
        compactSettings = {
          ...compactSettings,
          agent: {
            ...compactSettings.agent,
            softRatio: soft,
            snipRatio: snip,
            compactRatio: compact,
            forceRatio: force,
            effectiveCompactRatio: compact,
          },
        };
      },
    } as Partial<AppBindings> as AppBindings,
...
ok(compactRootEl.textContent?.includes("Compaction thresholds") === true, "compaction slider is visible without expanding a disclosure");
ok(compactRootEl.textContent?.includes("Compact") === true, "slider labels the compact thumb");
ok(compactRootEl.textContent?.includes("effective threshold is 75%") === true, "project override banner shows the active effective threshold");
ok(compactRootEl.textContent?.includes("≈80K") === true, "slider shows the configured ratio's token threshold on a 100K window");
ok(compactRootEl.textContent?.includes("Current threshold:") === false, "no legacy threshold summary row");
const earlyPresetButton = Array.from(compactRootEl.querySelectorAll("button")).find((button) => button.textContent?.includes("Early cleanup")) as HTMLButtonElement | undefined;
if (!earlyPresetButton) throw new Error("aggressive preset did not render");
await act(async () => {
  earlyPresetButton.click();
  await flushPromises();
});
eq(thresholdCalls.length, 1, "preset click persists one batch threshold update");
eq(thresholdCalls[0][0], 0.12, "preset sends the soft ratio");
eq(thresholdCalls[0][2], 0.2, "preset sends the compact ratio");
